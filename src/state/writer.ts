...
export function createWriterState() {
  ...
  const [selected, setSelected] = createSignal<SelectedBlock>({ primaryId: starterDocument.blocks[0].id });
  ...
  const storedWidth = Number.parseFloat(window.localStorage.getItem("blocks:inspector-width") ?? "");
  const [inspectorWidth, setInspectorWidth] = createSignal(Number.isFinite(storedWidth) ? Math.max(280, storedWidth) : 380);
  const [toasts, setToasts] = createSignal<Toast[]>([]);
  const [languageServerStatus, setLanguageServerStatus] = createSignal<LanguageServerStatus>("starting");
  const [languageServerMessage, setLanguageServerMessage] = createSignal("");
  const [typstDiagnostics, setTypstDiagnostics] = createSignal<TypstDiagnostic[]>([]);
  const [typstHighlights, setTypstHighlights] = createSignal<TypstHighlight[]>([]);
  const [autoCompile, setAutoCompileSignal] = createSignal(window.localStorage.getItem("blocks:auto-compile") === "true");
  let compileRequest = 0;

  const typstSource = createMemo(() => documentToTypst(document()));
  const activePrimary = createMemo(() => document().blocks.find((block) => block.id === selected().primaryId) ?? document().blocks[0]);
  ...
  const diagnosticsByBlock = createMemo(() => {
    ...
    for (const diagnostic of typstDiagnostics()) {
      const range = blockRanges().find(({ startLine, endLine }) => diagnostic.range.start.line >= startLine && diagnostic.range.start.line <= endLine);
      if (!range) continue;
      grouped.set(range.blockId, [...(grouped.get(range.blockId) ?? []), diagnostic]);
    }
    return grouped;
  });
  const diagnosticEntries = createMemo(() => typstDiagnostics().map((diagnostic) => {
    const range = blockRanges().find(({ startLine, endLine }) => diagnostic.range.start.line >= startLine && diagnostic.range.start.line <= endLine);
    const blockIndex = range ? document().blocks.findIndex((block) => block.id === range.blockId) : -1;
    const displayLine = range
      ? diagnostic.range.start.line - range.startLine + range.localStartLine + 1
  ...
  const highlightsByBlock = createMemo(() => {
    ...
    for (const highlight of typstHighlights()) {
      const range = blockRanges().find(({ startLine, endLine }) => highlight.line >= startLine && highlight.line <= endLine);
      if (!range) continue;
      grouped.set(range.blockId, [
        ...(grouped.get(range.blockId) ?? []),
        {
          ...
          start: highlight.start + (highlight.line === range.startLine ? range.localStartCharacter : 0),
        },
      ]);
    }
    return grouped;
  ...
  const removeLanguageServerListener = window.writer.onTypstLanguageServerEvent((event) => {
  ...
  createEffect(() => {
    ...
    languageServerTimer = window.setTimeout(async () => {
      ...
      if (result.available) {
        ...
        setLanguageServerMessage(result.message ?? "Install Tinymist to enable live Typst diagnostics.");
      }
    }, 180);
  });
  ...
  createEffect(() => {
    ...
    if (enabled) {
      autoCompileTimer = window.setTimeout(() => { void compileSource(source, false); }, 650);
    }
  });
  ...
  async function saveDocument() {
    try {
      const result = await window.writer.saveDocument(serializeDocument(document()));
      if (!result) return;
      setFilePath(result.filePath);
      showToast(`Saved ${fileName(result.filePath)}`);
    } catch (error) {
      showToast(error instanceof Error ? error.message : "Could not save document", "error");
    }
  }

  async function saveDocumentAs() {
    try {
      const result = await window.writer.saveDocumentAs(serializeDocument(document()));
      if (!result) return;
      setFilePath(result.filePath);
      showToast(`Saved ${fileName(result.filePath)}`);
    } catch (error) {
  ...
  function addPrimary(afterId?: string) {
    ...
    setDocument((current) => {
      const index = afterId ? current.blocks.findIndex((block) => block.id === afterId) : current.blocks.length - 1;
      next.position = defaultPrimaryPosition(index + 1);
      return { ...current, blocks: [...current.blocks.slice(0, index + 1), next, ...current.blocks.slice(index + 1)] };
    });
  ...
  function movePrimary(primaryId: string, direction: -1 | 1) {
    setDocument((current) => {
      ...
      blocks[destination] = translatePrimaryGroup(moving, index, primaryPosition(displaced, destination));
      blocks[index] = translatePrimaryGroup(displaced, destination, primaryPosition(moving, index));
  ...
  function addAlternative(primaryId: string) {
    ...
    setDocument((current) => {
      ...
      const primaryIndex = current.blocks.findIndex((block) => block.id === primaryId);
      alternative.position = defaultAlternativePosition(primaryPosition(primary, primaryIndex), primary.alternatives.length);
      return replacePrimary(current, primaryId, { ...primary, alternatives: [...primary.alternatives, alternative] });
    });
  ...
  function promoteAlternative(primaryId: string, alternativeId: string) {
    setDocument((current) => {
      const primary = getPrimary(current, primaryId);
      const promoted = primary?.alternatives.find((alternative) => alternative.id === alternativeId);
      if (!primary || !promoted) return current;
      const primaryIndex = current.blocks.findIndex((block) => block.id === primaryId);
      const alternativeIndex = primary.alternatives.findIndex((alternative) => alternative.id === alternativeId);
      const formerPrimary = {
        ...primary,
        position: alternativePosition(primary, primaryIndex, promoted, alternativeIndex),
        alternatives: [],
      };
  ...
  function moveAlternative(primaryId: string, alternativeId: string, direction: -1 | 1) {
    ...
    const alternative = primaries[from]?.alternatives.find((block) => block.id === alternativeId);
    ...
    setDocument((current) => {
      ...
      const withoutAlternative = replacePrimary(current, primaryId, {
        ...sourcePrimary,
        alternatives: sourcePrimary.alternatives.filter((block) => block.id !== alternativeId),
      });
  ...
  function updateBlockContent(primaryId: string, alternativeId: string | undefined, content: string) {
    updateBlock(primaryId, alternativeId, (block) => ({ ...block, content }));
  }

  function updateBlockPosition(primaryId: string, alternativeId: string | undefined, position: Point) {
    updateBlock(primaryId, alternativeId, (block) => ({ ...block, position }));
  }

  function updateBlockSize(primaryId: string, alternativeId: string | undefined, size: Size) {
    updateBlock(primaryId, alternativeId, (block) => ({ ...block, size }));
  }

  function updateBlock(primaryId: string, alternativeId: string | undefined, change: (block: Block) => Block) {
    setDocument((source) => {
      const primary = getPrimary(source, primaryId);
      if (!primary) return source;
      if (!alternativeId) return replacePrimary(source, primaryId, change(primary));
      return replacePrimary(source, primaryId, updateAlternative(primary, alternativeId, change));
    });
  }

  function movePrimaryGroup(primaryId: string, primaryIndex: number, position: Point) {
    setDocument((source) => {
      const primary = getPrimary(source, primaryId);
      return primary ? replacePrimary(source, primary.id, translatePrimaryGroup(primary, primaryIndex, position)) : source;
    });
  }
  ...
  async function compileSource(source: string, reveal: boolean) {
    ...
    if (!result.ok) {
      ...
      if (reveal) showToast(result.error || "Could not compile preview", "error");
      return;
    }
  ...
  async function completePrimary(primaryId: string, position: TypstPosition) {
    const range = blockRanges().find((candidate) => candidate.blockId === primaryId);
    if (!range || position.line < range.localStartLine) return [];
    const firstLineOffset = position.line === range.localStartLine ? range.localStartCharacter : 0;
  ...
  async function exportTypst() {
    try {
      ...
      showToast(error instanceof Error ? error.message : "Could not export Typst", "error");
    }
  }

  return {
    document, filePath, selected, setSelected, activePrimary, typstSource,
    previewUrl, previewState, previewError, rightPane, setRightPane,
    inspectorOpen, setInspectorOpen, inspectorWidth, setInspectorWidth, toasts,
    languageServerStatus, languageServerMessage, typstDiagnostics, typstHighlights, diagnosticsByBlock, diagnosticEntries, highlightsByBlock,
    autoCompile, setAutoCompile,
    newWindow: window.writer.newWindow, openDocument, saveDocument, saveDocumentAs, addPrimary, movePrimary,
    addAlternative, promoteAlternative, moveAlternative, updateBlockContent,
    updateBlockPosition, updateBlockSize, movePrimaryGroup, autoLayout,
    compilePreview, exportPdf, exportTypst,
    completePrimary,
  };
}
