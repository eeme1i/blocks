...
contextBridge.exposeInMainWorld("writer", {
  ...
  saveDocument: (contents: string) => ipcRenderer.invoke("document:save", contents),
  saveDocumentAs: (contents: string) => ipcRenderer.invoke("document:save-as", contents),
  ...
  exportTypst: (source: string) => ipcRenderer.invoke("typst:export-source", source),
  syncTypstLanguageServer: (source: string) => ipcRenderer.invoke("typst:lsp-sync", source),
  completeTypst: (source: string, line: number, character: number) => ipcRenderer.invoke("typst:lsp-complete", source, line, character),
