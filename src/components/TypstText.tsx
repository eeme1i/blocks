...
function highlightedLines(source: string, highlights: TypstHighlight[]) {
  ...
  return source.split("\n").map((line, lineNumber) => {
    ...
    let cursor = 0;
    const tokens = (tokensByLine.get(lineNumber) ?? []).sort(
    ...
    for (const token of tokens) {
      ...
      const end = Math.max(
        start,
        Math.min(line.length, token.start + token.length),
      );
      if (start > cursor) segments.push({ text: line.slice(cursor, start) });
      if (end > start)
        segments.push({ text: line.slice(start, end), type: token.type });
      cursor = end;
    }
...
export function localTypstHighlights(source: string) {
  ...
  const pattern =
    /(\/\/.*$)|("(?:\\.|[^"\\])*")|(https?:\/\/\S+)|(^\s*=+(?=\s))|(#(?:set|show|let|if|else|for|while|import|include|return|context)\b)|(#?[A-Za-z_][\w-]*(?=\s*\())|(\b\d+(?:\.\d+)?(?:pt|em|in|cm|mm|%)?\b)|([#*_`$[\]])/g;
...
export function HighlightedTypst(props: {
  source: string;
  highlights: TypstHighlight[];
}) {
  return (
    <For each={highlightedLines(props.source, props.highlights)}>
      {(line, lineNumber) => (
        <>
          <For each={line}>
            {(segment) => (
              <Show when={segment.type} fallback={segment.text}>
                {(type) => (
                  <span
                    class={`token-${type().replace(/[^a-zA-Z0-9_-]/g, "-")}`}
                  >
                    {segment.text}
                  </span>
                )}
          ...
          <Show when={lineNumber() < props.source.split("\n").length - 1}>
            {"\n"}
          </Show>
        </>
      )}
...
export function TypstEditor(props: {
  ...
  function requestCompletions(textarea: HTMLTextAreaElement) {
    ...
    completionTimer = window.setTimeout(async () => {
      let items = props.complete
        ? await props.complete(positionAt(value, cursor))
        : [];
        ...
        items = items
          ...
          .sort((left, right) => {
            const leftStarts =
              ...
              left.insertText.toLowerCase().startsWith(query);
            const rightStarts =
              right.label.toLowerCase().startsWith(query) ||
              right.insertText.toLowerCase().startsWith(query);
            return Number(rightStarts) - Number(leftStarts);
          });
      }
      if (items.length === 0 && prefix.startsWith("#")) {
        const normalized = prefix.toLowerCase();
        items = fallbackCompletions.filter((item) =>
  ...
  return (
    <div class="typst-editor">
      <pre
        ref={highlightedElement}
        class="node-highlight typst-highlight"
        aria-hidden="true"
      >
        <HighlightedTypst source={props.value} highlights={props.highlights} />
      </pre>
      <textarea
        ref={textareaElement}
        class="syntax-input"
        ...
        onInput={(event) => {
          ...
          requestCompletions(event.currentTarget);
        }}
        ...
        >
          <For each={completions()}>
            {(item, index) => (
              <button
                type="button"
                classList={{ active: index() === activeCompletion() }}
                onPointerDown={(event) => {
                  event.preventDefault();
